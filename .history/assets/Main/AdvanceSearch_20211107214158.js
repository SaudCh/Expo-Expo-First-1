...
import { View, Text, StyleSheet, Picker, CheckBox } from "react-native";
import {
  Checkbox,
  Input,
  NativeBaseProvider,
...
export default function AdvanceSearch(props) {
  ...
  const searchQ = (
    ...
    sold ? (sold = 1) : (sold = "");
    closure ? (closure = 1) : (closure = "");
    sale ? (sale = 1) : (sale = "");
    spa ? (spa = 1) : (spa = "");
    ghouse ? (ghouse = 1) : (ghouse = "");
    community ? (community = 1) : (community = "");
    waterfront ? (waterfront = 1) : (waterfront = "");
    gulf ? (gulf = 1) : (gulf = "");
    const item = {
      location,
      propertyType,

...
export default function Search(props) {
  ...
  let [minValue, setMinValue] = useState("Any");
  let [maxValue, setMaxValue] = useState("Any");
  let [city, setCity] = useState("Any");
  let [txt, setText] = useState("");

  const quickSearch = (txt, city, min, max) => {
    const item = { txt: txt, city: city, min: min, max: max };
    //console.log(item);
    const query = `https://first1.us/search.php?location=${txt}&mls_number=&min_price=${min}&max_price=${max}&property_type=&city=${city}&zipcode=&beds=Any&baths=Any&min_sq_ft=&max_sq_ft=&min_year=Any&garage=Any&just_listed=&include_sold=&foreclosure=&short_sale=&pool=&spa=&guest_house=&waterfront=&gated=&communities=&gulf_access=&ref=quick&sort=price-asc&pagination=get&page=1`;
        ...
        >
          ...
          >
            ...
            >
              ...
              <Picker.Item value="" label="No Limit" />
            </Picker>
          </View>
          <View
            style={{
          ...
          <Button
            ...
            onPress={() => quickSearch(txt, city, minValue, maxValue)}
          >
            Quick Search
          </Button>
        </View>

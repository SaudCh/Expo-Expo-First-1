...
export default function FeatureListings(props) {
  ...
  //const [favListing, setFavListing] = useState([1, 12, 3]);

  useFocusEffect(
    React.useCallback(() => {
      getUser();
      getMovies();
      //console.log(1 in favListing);
    })
  );
  ...
  //console.log(221054157 in favListing);
  //console.log(12 in favListing);
  const removeFav = async (itm) => {
    //console.log(itm);
    try {
      const response = await fetch(
        `https://first1.us/api/delFav.php?num=${itm}&user=${user}`
      );
      const json = await response.json();
      console.log(json);
    } catch (error) {
    ...
    <NativeBaseProvider>
      ...
      <ScrollView>
        ...
        {/* <View style={{ marginVertical: 100 }}>
          {221054157 in favListing ? <Text>True</Text> : <Text>False</Text>}
        </View> */}
